...
public class CuckooHash<K, V> {
    ...
    public void put(K key, V value) {
        ...
        int rehashAttemptCount = 0;
        final int MAX_ATTEMPTS = 2;
        Bucket<K, V> currentBuc = new Bucket<>(key, value);
        while (true) {
            int pos1 = hash1(currentBuc.getBucKey());
            int pos2 = hash2(currentBuc.getBucKey());
            K k = currentBuc.getBucKey();
            V v = currentBuc.getValue();
            if ((table[pos1] != null && table[pos1].getBucKey().equals(k) && table[pos1].getValue().equals(v)) ||
                    (table[pos2] != null && table[pos2].getBucKey().equals(k) && table[pos2].getValue().equals(v))) {
                return;
            }
            int currentPos = hash1(currentBuc.getBucKey());
            boolean inserted = false;

            for (int i = 0; i < CAPACITY; i++) {
                Bucket<K, V> kickedOutBuc = table[currentPos];
                table[currentPos] = currentBuc;

                if (kickedOutBuc == null) {
                    inserted = true;
                    break;
                }
                currentBuc = kickedOutBuc;
                int altPos1 = hash1(currentBuc.getBucKey());
                int altPos2 = hash2(currentBuc.getBucKey());
                if (currentPos == altPos1) {
                    currentPos = altPos2;
                } else {
                    currentPos = altPos1;
                }
            ...
            if (rehashAttemptCount >= MAX_ATTEMPTS) {
                throw new IllegalStateException("Max rehash attempts reached. Cuckoo hash table stability failure.");
            }
            rehashAttemptCount++;
            rehash();
        }
